...
DATABASES = {
    'default': {
        ...
        'ENGINE': 'django.db.backends.postgresql_psycopg2', # Add 'postgresql_psycopg2', 'mysql', 'sqlite3' or 'oracle'.
        'NAME': 'tax_invoice',                      # Or path to database file if using sqlite3.
...
STATIC_ROOT =os.path.join(HERE , 'static2').replace('\\','/')
...
LOGGING = {
	...
	'handlers': {
		...
		'tax_handler': {
			'level': 'DEBUG',
			'filename':'logs/tax.log',
			'class': 'logging.handlers.RotatingFileHandler',
			'formatter':'tax_formatters',
			}
	},
...
CONFIG_DIR = os.path.join(HERE , 'config').replace('\\','/')